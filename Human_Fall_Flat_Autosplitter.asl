...
init		// called when the script finds the game process
{
	...
	foreach (var page in game.MemoryPages())
	{
		...
		if (cheatStartAddress == IntPtr.Zero)
		{
			...
			(
				new SigScanTarget(0,
				"55 8B EC 57 83 EC 04 8B 7D 08 85 FF 0F 84 2E 03 00 00 83 EC 0C 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 10 89 78 10 C7 40 14")
			);
		}
	...
	if (settings["noJump%"])
	{
		foreach (var page in game.MemoryPages())
		{
			...
			if (humanEnableAddress == IntPtr.Zero)
			{
				...
				(
					new SigScanTarget(0,
					"55 8B EC 57 83 EC 04 8B 7D 08 8B 05 ?? ?? ?? ?? 83 EC 08 57 50 39 00 E8 ?? ?? ?? ?? 83 C4 10 B8 ?? 3A ?? ?? 89 38 BA ?? ?? ?? ?? 83 EC 0C 57 E8 ?? ?? ?? ?? 83 C4 10 89 47 1C BA ?? ?? ?? ?? 83 EC 0C 57 E8 ?? ?? ?? ?? 83 C4 10 89 47 18 BA ?? ?? ?? ?? 83 EC 0C 57 E8 ?? ?? ?? ?? 83 C4 10 89 47 20 BA ?? ?? ?? ?? 83 EC 0C 57 E8 ?? ?? ?? ?? 83 C4 10 89 47 14 8D 65 FC 5F C9 C3")
				);
			}
			if (humanEnableAddress != IntPtr.Zero)
				break;
