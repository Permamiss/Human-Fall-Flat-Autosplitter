...
init		// called when the script finds the game process
{
	...
	foreach (var page in game.MemoryPages())
	{
		...
		if (gameInitializeAddress == IntPtr.Zero)
		{
			...
			(
				new SigScanTarget(0,
				"55 8B EC 57 83 EC 24 8B 7D 08 B8 20 AD ?? ?? 89 38 C7 04 24 00 00 00 00 8D 6D 00 E8 A8 00 00 00 BA ?? ?? ?? ?? 8B C0 E8 ?? ?? FF FF BA ?? ?? ?? ?? 89 3C 24 8D 6D 00 E8 ?? ?? FF FF C7 04 24 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 45 F8 89 04 24 90 E8 84 02 00 00 8B 45 F8 89 47 34 BA ?? ?? ?? ?? 89 3C 24 90 E8 28 16 00 00 89 47 20 C7 47 5C 00 00 00 00 8B 47 10 8B 40 0C 48 89 47 40 8D 65 FC 8D 65 FC 5F C9 C3")
			);
		}
	...
	if (settings["noJump%"])
	{
		foreach (var page in game.MemoryPages())
		{
			...
			if (humanEnableAddress == IntPtr.Zero)
			{
				...
				(
					new SigScanTarget(0,
					"55 8B EC 57 83 EC 24 8B 7D 08 8B 05 9C 55 ?? ?? 89 7C 24 04 89 04 24 39 00 8B C0 E8 ?? ?? ?? ?? B8 98 55 ?? ?? 89 38 BA ?? ?? ?? ?? 89 3C 24 E8 ?? ?? ?? ?? 89 47 1C BA ?? ?? ?? ?? 89 3C 24 E8 ?? ?? ?? ?? 89 47 18 BA ?? ?? ?? ?? 89 3C 24 E8 ?? ?? ?? ?? 89 47 20 BA ?? ?? ?? ?? 89 3C 24 E8 4C FE FF FF 89 47 14 8D 65 FC 8D 65 FC 5F C9 C3")
				);
			}
			if (humanEnableAddress != IntPtr.Zero)
				break;
